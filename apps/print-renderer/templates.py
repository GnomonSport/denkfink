...
# Public URL where the archive app is deployed. Set ARCHIVE_BASE_URL in the
# print-renderer's environment to enable QR codes on printed cards; leave unset
# to skip QR generation.
...
def render_dictionary_image(data: dict, config: dict = None, style: str = "dictionary") -> Image.Image:
    ...
    # QR code linking to archive page (skipped if ARCHIVE_BASE_URL is not set)
    if definition_id and ARCHIVE_BASE_URL:
        y += date_h + qr_gap
